...
public class LocalTerminalHelpTest {
    ...
    void shouldDescribeLocalTerminalCommands() {
        assertThat(LocalTerminalHelp.text())
                ...
                .contains("/config path|show|edit|check|set <key> <value>")
                ...
                .contains("/gateway install|uninstall")
                .doesNotContain("env-path")
                .contains("/pairing list|pending|approved|approve|revoke|clear-pending")
                .contains("/fallback list|add|remove|clear")
                .contains("/postinstall、/login、/auth、/fallback、/secrets、/proxy、/mcp、/send")
                ...
                .contains("/approve auto [status|on|off]")
                .contains("/deny [确认编号]")
