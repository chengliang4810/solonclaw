...
public final class LocalTerminalHelp {
    ...
    public static String text() {
        return "本地终端命令：\n"
                ...
                + "/gateway install|uninstall - 查看单实例部署下的服务安装裁剪说明\n"
                + "/pairing list|owner claim|owner clear - 查看或管理渠道主人绑定\n"
                ...
                + "/postinstall、/login、/auth、/fallback、/secrets、/proxy、/mcp、/send - 查看对应本地配置或裁剪说明\n"
                ...
                + "/goal [status|show|pause|resume|clear|stop|done|wait <pid>|unwait|<目标> --max N] - 管理跨轮长目标循环\n"
                + "/subgoal [<text>|remove <n>|clear] - 管理当前目标的补充准则\n"
                + "/recap [条数] - 显示恢复会话用的紧凑历史摘要\n"
                + "/trajectory [save] [原始问题] - 导出或保存会话轨迹 JSON\n"
                + "/compact [重点] - 压缩当前会话上下文\n"
                ...
                + "/approve auto [status|on|off] - 查看或切换当前会话危险命令自动批准模式\n"
                + "/deny [确认编号] - 拒绝危险命令审批\n"
