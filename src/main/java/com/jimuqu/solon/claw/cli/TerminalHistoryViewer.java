...
public class TerminalHistoryViewer {
    ...
    public String render(String sessionId, String input) {
        ...
        buffer.append("使用：/history <条数> 查看更多，/sessions 浏览其他会话。");
    ...
    private List<HistoryEntry> entries(SessionRecord session, int limit) throws Exception {
        ...
        for (ChatMessage message : messages) {
            ...
            all.add(new HistoryEntry(all.size() + 1, roleLabel(message.getRole()), trim(content)));
        }
        if (all.size() <= limit) {
            return all;
        }
    ...
    private static class HistoryEntry {
        ...
        private HistoryEntry(int index, String role, String preview) {
            this.index = index;
            this.role = role;
