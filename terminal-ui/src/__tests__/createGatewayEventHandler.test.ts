...
describe('createGatewayEventHandler', () => {
  ...
  it('surfaces self-improvement review summaries as a persistent system line', () => {
    ...
    onEvent({
      payload: { text: "💾 Self-improvement review: Skill 'solonclaw-release' patched" },
      type: 'review.summary'
    } as any)
  ...
  it('annotates gateway.start_timeout with stderr tail lines so users can diagnose without /logs', () => {
    ...
    onEvent({
      payload: {
        ...
        stderr_tail:
          '[startup] timed out\nModuleNotFoundError: No module named openai\nFileNotFoundError: ~/.solonclaw/config.yaml'
      },
  ...
  it('prefers raw text over Rich-rendered ANSI on message.complete (#16391)', () => {
    ...
    const rendered = '\u001b[33msolonclaw here.\u001b[0m\n\n\u001b[2mLine two.\u001b[0m'
  ...
  it('keeps streamed <think> tags out of visible message deltas', () => {
    ...
    onEvent({ payload: { text: '<think>内部推理' }, type: 'message.delta' } as any)
    onEvent({ payload: { text: '</think>\n\n最终答复' }, type: 'message.delta' } as any)
    turnController.flushStreamingSegment()
  ...
  it('shows setup panel for missing provider startup error', () => {
    ...
    onEvent({
      payload: {
        message:
          'agent init failed: No LLM provider configured. Run `solonclaw model` to select a provider, or run `solonclaw setup` for first-time configuration.'
      },
      type: 'error'
    } as any)
  ...
  it('on gateway.ready surfaces a ready startup status after the forged session resolves', async () => {
    ...
    ctx.session.STARTUP_RESUME_ID = ''
    ctx.session.newSession = vi.fn(async () => {
      patchUiState({ sid: 'sess-ready', status: 'ready' })
    ...
    await vi.waitFor(() => expect(getUiState().sid).toBe('sess-ready'))
    expect(getUiState().status).toBe('ready')
  })
  ...
  it('on gateway.ready does not block new session on a hung auto-resume config read', async () => {
    const appended: Msg[] = []
    const newSession = vi.fn()
    const resumeById = vi.fn()
    const ctx = buildCtx(appended)
  ...
  it('drops stale reasoning/tool/todos events after ctrl-c until the next message starts', () => {
    // Repro: ctrl-c interrupts, but late reasoning/tool events from the
    // still-winding-down agent loop kept populating the UI for ~1s, making it
    // look like the interrupt had been ignored.
    //
