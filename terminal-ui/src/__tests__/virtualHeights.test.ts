...
describe('virtual height estimates', () => {
  ...
  it('uses compound user prompt width when estimating user message wrapping', () => {
    ...
    const compactPromptHeight = estimatedMsgHeight(msg, 26, {
      compact: false,
      details: false,
      userPrompt: '❯'
    })

    const compoundPromptHeight = estimatedMsgHeight(msg, 26, {
      compact: false,
      details: false,
      userPrompt: 'Ψ >'
    })

    expect(compoundPromptHeight).toBeGreaterThanOrEqual(compactPromptHeight)
    expect(compoundPromptHeight - compactPromptHeight).toBeLessThanOrEqual(1)
  })
