...
const modelOptionsText = (r: ModelOptionsResponse) => {
  const lines = [
    `current: ${r.model || '(unset)'}`,
    `provider: ${r.provider || '(unset)'}`,
    '',
    'providers:'
  ]

  for (const provider of r.providers ?? []) {
    const marker = provider.is_current ? '*' : '-'
    const auth = provider.authenticated ? 'configured' : 'missing key'
    const models = provider.models?.length ? provider.models.join(', ') : '(no models)'

    lines.push(`${marker} ${provider.slug} · ${provider.name} · ${auth}`)
    lines.push(`  ${models}`)
  }
