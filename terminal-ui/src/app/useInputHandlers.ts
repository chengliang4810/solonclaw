...
export function useInputHandlers(ctx: InputHandlerContext): InputHandlerResult {
  ...
  // Wheel acceleration uses inter-event timing to drive step size; direction
  // flips reset. wheelStep (WHEEL_SCROLL_STEP) is the base; final rows =
  // wheelStep × accelMult. State mutates in place across renders.
  ...
  const cancelOverlayFromCtrlC = () => {
    ...
    if (overlay.channelSetup) {
      return patchOverlayState({ channelSetup: false })
    }
