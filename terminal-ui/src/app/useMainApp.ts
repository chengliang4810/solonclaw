...
import { type ScrollBoxHandle, useApp, useHasSelection, useSelection, useStdout, useTerminalTitle } from '@solonclaw/ink'
...
export function useMainApp(gw: GatewayClient) {
  ...
  const answerApproval = useCallback(
    (choice: string) =>
      respondWith('approval.respond', { approval_id: overlay.approval?.approvalId, choice, session_id: ui.sid }, () => {
  ...
  const appActions = useMemo(
    () => ({
      ...
      newPromptSession,
      onModelSelect,
      onSetupDoctor: () => slashRef.current('/doctor'),
  ...
  const cwd = ui.info?.cwd || process.env.SOLONCLAW_CWD || process.cwd()
