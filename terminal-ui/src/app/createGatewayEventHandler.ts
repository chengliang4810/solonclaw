...
export function createGatewayEventHandler(ctx: GatewayEventHandlerContext): (ev: GatewayEvent) => void {
  ...
  const STARTUP_CONFIG_TIMEOUT_MS = 250
  ...
  const getStartupFullConfig = (): Promise<ConfigFullResponse | null> =>
    Promise.race([
      getFullConfigOnce(),
  ...
  const handleReady = (skin?: GatewaySkin) => {
    ...
    // forging a brand-new one.  Mirrors classic CLI's `solonclaw -c` /
    // `solonclaw` muscle memory and addresses the audit's "session
    ...
    getStartupFullConfig()
      .then(cfg => {
        ...
        if (!cfg?.config?.display?.tui_auto_resume_recent) {
          ...
          recordParentLifecycle('[startup] newSession from gateway.ready')
          newSession()
          scheduleStartupPrompt()
