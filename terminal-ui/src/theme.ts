...
// Mirrors @solon-claw/ink's colorize.ts. Keep local: app code compiles from
// terminal-ui/src, while @solon-claw/ink is bundled separately from packages/.
...
// Terminal. Explicit SOLONCLAW_TUI_THEME / COLORFGBG signals above still win,
...
// `SOLONCLAW_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `SOLONCLAW_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `SOLONCLAW_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `SOLONCLAW_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default SolonClaw palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.SOLONCLAW_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.SOLONCLAW_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.SOLONCLAW_TUI_BACKGROUND ?? '')
