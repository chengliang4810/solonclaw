...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.SOLONCLAW_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.solonclaw', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `solonclaw-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
    ...
    const autoEnabled = /^(?:1|true|yes|on)$/i.test((process.env.SOLONCLAW_AUTO_HEAPDUMP ?? '').trim())
...
async function pruneHeapdumps(dir: string): Promise<void> {
  const raw = process.env.SOLONCLAW_HEAPDUMP_MAX_BYTES?.trim()
