 ...
 * solon-claw-ink sets ``key.meta`` for plain Alt/Option on every platform
 ...
 * ``opt`` spellings match the backend voice shortcut normalizer, so one
 * ``voice.record_key`` value binds the same shortcut in both runtimes.
 * The ``super`` /
 ...
 * modifier — but solon-claw-ink reports Alt as ``key.meta`` on many
...
export const parseVoiceRecordKey = (raw: unknown): ParsedVoiceRecordKey => {
  ...
  // On macOS solon-claw-ink reports Alt as ``key.meta``, which ``isActionMod``
...
export const isVoiceToggleKey = (
  ...
  switch (configured.mod) {
    case 'alt':
      ...
      // alt binding.
      //
      // Bare Escape on solon-claw-ink can arrive as ``key.meta=true`` on some
    ...
    case 'ctrl':
      ...
      // ``key.meta`` is solon-claw-ink's Alt signal and accepting it would
