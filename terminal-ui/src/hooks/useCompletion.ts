...
export function completionRequestForInput(
  ...
  if (isSlashCommand) {
    return { method: 'complete.slash', params: { text: input }, replaceFrom: 0 }
  }
...
export function replacementStartForCompletion(
  method: 'complete.path' | 'complete.slash',
  replaceFrom: number | undefined,
  fallback: number
) {
  if (method === 'complete.slash') {
    return typeof replaceFrom === 'number' ? Math.max(0, replaceFrom) : fallback
  }
    ...
    const t = setTimeout(() => {
      ...
      gw.request<CompletionResponse>(request.method, request.params)
        .then(raw => {
          ...
          setCompIdx(0)
          setCompReplace(replacementStartForCompletion(request.method, r?.replace_from, request.replaceFrom))
        })
        .catch((e: unknown) => {
          if (ref.current !== input) {
            return
