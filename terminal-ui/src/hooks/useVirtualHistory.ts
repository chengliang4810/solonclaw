...
export function useVirtualHistory(
  ...
  if (frozenRange) {
    ...
    if (vp <= 0) {
      ...
      // covered. Cap the span at 3×viewport so pendingDelta growing unbounded
      // (MX Master free-spin) doesn't blow the mount budget; the clamp
      // (setClampBounds) shows edge-of-mounted content during catch-up.
      const MAX_SPAN = vp * 3
