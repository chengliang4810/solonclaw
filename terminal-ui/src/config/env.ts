...
export const STARTUP_RESUME_ID = (process.env.SOLONCLAW_TUI_RESUME ?? '').trim()
export const STARTUP_QUERY = (process.env.SOLONCLAW_TUI_QUERY ?? '').trim()
...
export const NO_CONFIRM_DESTRUCTIVE = truthy(process.env.SOLONCLAW_TUI_NO_CONFIRM)

const inlineOverride = parseToggle(process.env.SOLONCLAW_TUI_INLINE)
...
// less fragile. Override explicitly with SOLONCLAW_TUI_INLINE=0/1.
...
// synthetic timer).
export const SHOW_FPS = truthy(process.env.SOLONCLAW_TUI_FPS)
