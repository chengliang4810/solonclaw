    ...
    <Box alignItems="flex-start" flexDirection="column" flexShrink={0}>
      ...
      {overlay.setupPanel && (
        <FloatBox color={theme.color.border}>
          <SetupPanel
            gw={gw}
            onChannel={() => patchOverlayState({ channelSetup: true, modelPicker: false, setupPanel: false })}
            onClose={() => patchOverlayState({ setupPanel: false })}
            onDoctor={() => {
              patchOverlayState({ setupPanel: false })
              onSetupDoctor()
            }}
            onModel={() => patchOverlayState({ channelSetup: false, modelPicker: true, setupPanel: false })}
            t={theme}
          />
        </FloatBox>
      )}
      ...
      {overlay.channelSetup && (
        <FloatBox color={theme.color.border}>
          <ChannelSetup gw={gw} onClose={() => patchOverlayState({ channelSetup: false })} sessionId={sid} t={theme} />
        </FloatBox>
      )}
