...
if (!process.stdin.isTTY) {
  console.log('solonclaw-tui: no TTY')
  process.exit(0)
}
...
setupGracefulExit({
  ...
  onError: (scope, err) => {
    ...
    process.stderr.write(`solonclaw-tui lifecycle ${scope}: ${message.slice(0, 2000)}\n`)
  },
  onSignal: signal => {
    ...
    resetTerminalModes()
    process.stderr.write(`solonclaw-tui lifecycle: received ${signal}\n`)
  }
})
...
const stopMemoryMonitor = startMemoryMonitor({
  onCritical: (snap, dump) => {
    ...
    resetTerminalModes()
    process.stderr.write(`solonclaw-tui lifecycle: memory critical exit heap=${formatBytes(snap.heapUsed)} rss=${formatBytes(snap.rss)}\n`)
    process.stderr.write(dumpNotice(snap, dump))
    process.stderr.write('solonclaw-tui: exiting to avoid OOM; restart to recover\n')
    process.exit(137)
  },
  ...
  onWarn: snap => {
    ...
    process.stderr.write(
      `solonclaw-tui: heap climbing fast (${formatBytes(snap.heapUsed)}) — a large tool output or long session may be straining memory\n`
    )
  }
})

if (process.env.SOLONCLAW_HEAPDUMP_ON_START === '1') {
