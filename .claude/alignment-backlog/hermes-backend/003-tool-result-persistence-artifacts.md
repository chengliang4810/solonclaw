...
## Hermes 参考
- `/Users/chengliang/code-repositories/hermes-agent/tools/tool_result_storage.py`
- `/Users/chengliang/code-repositories/hermes-agent/agent/tool_executor.py`
...
## 当前缺口
Hermes 会把大型工具输出外置存储，并在 live transcript 中保留 preview/path；当前项目主要依赖 SQLite session snapshot 与 artifact 服务，仍需确认超大工具结果、可回放工具内容、recap/trajectory 输出是否需要同级 spill-to-file 或归一化。
