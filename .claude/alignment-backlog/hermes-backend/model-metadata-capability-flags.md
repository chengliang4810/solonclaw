# model-metadata-capability-flags
...
## 现有补充草案
- 已有同主题草案：`../external-backend/011-model-metadata-capability-flags.md`
...
## 当前缺口
当前项目缺少与 Hermes 对齐的细粒度模型元数据层，尤其是 audio、attachment、multimodal 以及来源标记；现有能力更多依赖静态配置或粗粒度推断，无法稳定驱动模型选择、图像路由和降级策略。
...
## 验证方式
补齐元数据字段后，运行模型目录与能力解析相关测试，重点验证 vision/audio/attachment 标记与来源回退逻辑。
