# SolonClaw 陪伴型 Agent 设计稿

## 1. 目标与对应对标能力点

本设计对应以下对标能力面，并严格受 `AGENTS.md` 约束：
...
本设计不追求“把 SolonClaw 全量搬进来”，而是抽取其中最关键的 companion 机制，并用更适合 Java / Solon / Solon AI 的方式落地到 `solon-claw`。
...
## 4. SolonClaw Companion 机制在本项目中的定义

SolonClaw 的 companion 感不是单靠一段 prompt，而是依赖以下五层：
...
第一版不追求把每个 SolonClaw UI 细节完全做齐，但 companion 需要的投递侧行为应最小闭环：
...
### 11.1 SolonClaw 对 agent 的定义在本项目中的等价物
