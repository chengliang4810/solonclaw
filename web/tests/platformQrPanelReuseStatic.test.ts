...
assert.ok(sharedPanel.includes("state.status === 'confirmed' ? state.userId : ''"), 'Shared QR panel should show user id only after confirmation')
assert.ok(zh.includes("qrAccountId: '账号 ID'"), 'Chinese locale should label QR account id')
assert.ok(zh.includes("qrUserId: '用户 ID'"), 'Chinese locale should label QR user id')
...
assert.ok(en.includes("qrUserId: 'User ID'"), 'English locale should label QR user id')
