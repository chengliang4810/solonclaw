...
const memoryViewFile = new URL('../src/views/solonclaw/MemoryView.vue', import.meta.url)
...
const memoryView = readFileSync(memoryViewFile, 'utf8')
const personaFileView = readFileSync(personaFileViewFile, 'utf8')

assert.ok(memoryView.includes('MarkdownDocumentPanel'), 'MemoryView should delegate document preview/edit UI')
assert.ok(personaFileView.includes('MarkdownDocumentPanel'), 'PersonaFileView should delegate document preview/edit UI')
assert.equal(
  (memoryView.match(/<MarkdownDocumentPanel/g) || []).length,
  1,
  'MemoryView should render exactly one shared document panel',
)
assert.equal(
...
assert.ok(!memoryView.includes('class="edit-textarea"'), 'MemoryView should not keep duplicated textarea markup')
assert.ok(!personaFileView.includes('class="edit-textarea"'), 'PersonaFileView should not keep duplicated textarea markup')
assert.ok(memoryView.includes('replace(/§/g'), 'MemoryView should keep memory section display transformation')
