...
assert.ok(jobForm.includes('options.push({ label: value, value, disabled: false })'), 'job form should keep configured custom toolsets selectable')
...
assert.ok(!jobForm.includes('hasCompleteModelBinding'), 'Cron editing should not contain old partial-binding migration logic')
assert.ok(!jobForm.includes('formData.value.base_url'), 'Cron editing should not expose a task-level base URL')
assert.ok(!jobsApi.includes('base_url'), 'Cron API types should source endpoints only from registered Providers')
assert.ok(
...
assert.ok(!jobForm.includes("t('models.unregisteredModel'"), 'Cron should only expose registered models')
