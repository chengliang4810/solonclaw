...
assert.ok(sessionsApi.includes("sessionSubpath(id, 'trajectory'"), 'trajectory fetch should call backend trajectory endpoint')
assert.ok(view.includes('fetchSessionRecap(sessionId)'), 'runs view should load recap for the captured session')
assert.ok(view.includes('fetchSessionTrajectory(sessionId)'), 'runs view should load trajectory for the captured session')
