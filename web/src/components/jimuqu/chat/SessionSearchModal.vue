...
import { fetchSessions, searchSessions, type SessionSearchResult, type SessionSummary } from '@/api/jimuqu/sessions'
...
async function openItem(item: SearchItem) {
  ...
  if (router.currentRoute.value.name !== 'Jimuqu.chat') {
    await router.push({ name: 'Jimuqu.chat' })
  }
}
