...
function buildWsUrl(): string {
  ...
  if (base) {
    return `${wsProtocol}//${new URL(base).host}/api/jimuqu/terminal${token ? `?token=${encodeURIComponent(token)}` : ""}`;
  }
  ...
  const host = import.meta.env.DEV
    ...
    : location.host;
  return `${wsProtocol}//${host}/api/jimuqu/terminal${token ? `?token=${encodeURIComponent(token)}` : ""}`;
}
