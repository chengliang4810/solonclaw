...
export async function fetchCuratorReports(limit = 20): Promise<CuratorReportSummary[]> {
  const res = await request<{ reports: CuratorReportSummary[] }>(`/api/jimuqu/curator?limit=${limit}`)
  return res.reports || []
}

export async function runCurator(force = true) {
  return request(`/api/jimuqu/curator/run?force=${force}`, { method: 'POST' })
}

export async function fetchCuratorReport(reportId: string) {
  return request(`/api/jimuqu/curator/${reportId}`)
}
