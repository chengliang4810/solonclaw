...
export async function fetchRunEvents(runId: string): Promise<AgentRunEvent[]> {
  const res = await request<{ events: AgentRunEvent[] }>(`/api/jimuqu/runs/${runId}/events`)
  return res.events || []
}

export async function fetchRunDetail(runId: string): Promise<RunDetail> {
  return request<RunDetail>(`/api/jimuqu/runs/${runId}/detail`)
}

export async function fetchRunTools(runId: string): Promise<ToolCall[]> {
  const res = await request<{ tools: ToolCall[] }>(`/api/jimuqu/runs/${runId}/tools`)
  return res.tools || []
}

export async function fetchRunSubagents(runId: string): Promise<SubagentRun[]> {
  const res = await request<{ subagents: SubagentRun[] }>(`/api/jimuqu/runs/${runId}/subagents`)
  return res.subagents || []
}

export async function fetchRecoverableRuns(limit = 50): Promise<AgentRun[]> {
  const res = await request<{ runs: AgentRun[] }>(`/api/jimuqu/runs/recoverable?limit=${limit}`)
  return res.runs || []
}

export async function controlRun(runId: string, command: string, payload: Record<string, unknown> = {}) {
  return request(`/api/jimuqu/runs/${runId}/control`, {
    method: 'POST',
